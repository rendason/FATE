...
class Guest(HeteroGradientBase):
    ...
    def compute_gradient_procedure(self, data_instances, cipher_operators, model_weights, optimizer,
        ...
        guest_gradient = compute_gradient(data_instances,
                                               fore_gradient,
                                               model_weights.fit_intercept)
        if optimizer is not None:
            guest_gradient = optimizer.add_regular_to_grad(guest_gradient, model_weights)

        host_gradients = self.get_host_gradient(suffix=current_suffix)
        ...
        LOGGER.debug("In guest compute_gradient_procedure, delta_grad: {}".format(
            delta_grad
        ))
        separate_optim_gradient = self.separate(delta_grad, size_list)
        LOGGER.debug("In guest compute_gradient_procedure, separated gradient: {}".format(
            separate_optim_gradient
        ))
        host_optim_gradients = separate_optim_gradient[: -1]
        guest_optim_gradient = separate_optim_gradient[-1]

        self.remote_local_gradient(host_optim_gradients, current_suffix)

        return guest_optim_gradient, fore_gradient, self.host_forwards
    ...
    def remote_local_gradient(self, host_optim_gradients, suffix=tuple()):
...
class Host(HeteroGradientBase):
    ...
    def update_gradient(self, unilateral_gradient, suffix=tuple()):
        self.unilateral_gradient_transfer.remote(unilateral_gradient, role=consts.GUEST, idx=0, suffix=suffix)
